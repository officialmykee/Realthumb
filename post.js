...
const CSS = `
  ...
  .feed-bg {
    background: #e8ebf0;
    /* white strip at the very top so first card blends into stories */
  ...
  /* First card sits flush against the stories — no top radius, no gap */
  .feed-inner .card:first-child {
...
function mount() {
  ...
  // Notify index.html which element is the scrollable feed,
  // so it can show/hide stories based on scroll direction.
  const scrollEl = root.querySelector('.feed-bg');
  if (scrollEl) {
    window.dispatchEvent(new CustomEvent('feed-mounted', { detail: { scrollEl } }));
  }
}
